...
export default class Gantt {
  ...
  make_grid_header() {
    console.log("🚀 ~ Gantt ~ make_grid_header ~ make_grid_header:");
    const curHeader = document.querySelector(".grid-header");
    if (curHeader) document.querySelector(".grid-header").remove();
  ...
  highlightWeekends() {
    ...
    for (
      ...
      if (d.getDay() === 0 || d.getDay() === 6) {
        const x =
          (date_utils.diff(d, this.gantt_start, "hour") / this.options.step) *
